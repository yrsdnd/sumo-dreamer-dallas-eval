...
class MLP(nn.Module):
    def __init__(
        ...
        act = getattr(torch.nn, act)
        self._dist = dist
        # std tensor lives on the default device; the owning Module.to(device)
        # will migrate it. Prevents CUDA assertions when MultiEncoder (which
        # doesn't thread ``device`` through to its inner MLP) runs on CPU.
